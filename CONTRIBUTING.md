...
We use Prow-style bot commands to streamline issue and pull request management. Simply comment with commands starting with `/` on any issue or PR.

### Available Commands

**For Everyone:**
- `/assign` - Assign yourself to an issue/PR
- `/assign @username` - Assign someone else
- `/unassign` - Remove assignment
- `/cc @username` - Request review or input
- `/uncc @username` - Remove cc
...
- `/lgtm` - Mark PR as "looks good to me" (ready to merge)
- `/approve` - Approve PR (maintainers only)
- `/hold` - Prevent PR from merging (maintainers only)
- `/unhold` - Remove merge hold (maintainers only)
